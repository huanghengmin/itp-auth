...
//        {header: "服务编号", width: 50,align:'center', dataIndex: 'id',sortable:true,menuDisabled:true},
        ...
        {header: "服务地址", width: 100, dataIndex: 'ipPort',align:'center',sortable:true,menuDisabled:true},
        {header: "创建时间", width: 100, dataIndex: 'createTime',align:'center',sortable:true,menuDisabled:true},
        {
            ...
            width:100,
            renderer:function(v){
                return "<a href='javascript:;'  onclick='getServerUser()'>服务账号</a>  <a href='javascript:;'  onclick='restart()'>重启服务</a>";
            }
        }
    ]);
...
//        bbar:pagebar
    ...
    var ip_store = new Ext.data.Store({
        url:"ServerUserAction_getIps.action",
        reader:ip_reader
    });
    ip_store.load();
            ...
            items:[
                {
                    ...
                    listeners:{
                        'select':function(){
                            if(this.getValue() == 0){
                                Ext.getCmp('insert.port').setValue('21');
                            }else if(this.getValue() == 1){
                                Ext.getCmp('insert.port').setValue('445');
                            }
                        }
                    }
                },{
                    ...
                    items:[{
                    ...
                    },
                        {
                            ...
                            listeners:{
                                'select':function(){
                                    if(this.getValue() == 0){
                                        Ext.getCmp('update.port').setValue('21');
                                    }else if(this.getValue() == 1){
                                        Ext.getCmp('update.port').setValue('445');
                                    }
                                }
                            }
                        },{
                            ...
                            displayField:"ip",valueField:"ip",
                            store:ip_store ,
                            value:(item.data.ipPort).split(":")[0]
                        },{
                            id:'update.port',
                            fieldLabel:'服务端口',
                            name:'port',
                            regex:/^([0-9]|[1-9]\d|[1-9]\d{2}|[1-9]\d{3}|[1-5]\d{4}|6[0-4]\d{3}|65[0-4]\d{2}|655[0-2]\d|6553[0-5])$/,
                            regexText:'请输入正确的端口,格式为port',
                            value:(item.data.ipPort).split(":")[1]
                        }/*{
                            id:'update.ipPort',
                            fieldLabel:"IP端口",
...
function restart(){
    ...
    myMask.show();
    Ext.Ajax.request({
        url : 'ServerUserAction_restart.action',             // 删除 连接 到后台
        method:'POST',
        success : function(r,o){
            myMask.hide();
            var respText = Ext.util.JSON.decode(r.responseText);
...
function getServerUser(){
    ...
    if(selModel.hasSelection()){
        ...
        Ext.each(selections,function(item){
            ...
            var store = new Ext.data.Store({
                proxy: new Ext.data.HttpProxy({url:'ServerUserAction_selectUsers.action',method:'POST'}),
//                proxy: new Ext.data.HttpProxy({url:'ServerUserAction_selectUsers.action?serverName=' + item.data.serverName,method:'POST'}),
//                params:{serverName:item.data.serverName,serverId:item.data.id},
                reader: new Ext.data.JsonReader({
                    totalProperty:"total",
                    root: 'rows'
                }, resourceRecord)
            ...
            store.load({
                params:{
                    start:start,limit:pageSize,
                    serverName:item.data.serverName,serverId:item.data.id
                }
            });
            var rowNumber = new Ext.grid.RowNumberer();         //自动 编号
            var resourceCm = new Ext.grid.ColumnModel([
                ...
                {header: "服务权限", width: 120,align:'center', dataIndex: 'permission',sortable:true,menuDisabled:true,renderer:userPermission},
                ...
                {header: "创建时间", width: 120, dataIndex: 'createTime',align:'center',sortable:true,menuDisabled:true},
                {
                    ...
                    width:100,
                    renderer:function(v){
                        return "<a href='javascript:;'  onclick='updateServerUser("+ item.data.serverType +")'>修改</a>  <a href='javascript:;'  onclick='changePwd()'>修改密码</a>";// <a href='javascript:;' onclick='getClients()'>分配权限</a> <a href='javascript:;' onclick='deleteServerUser()'>删除</a>";
                        /*if(item.data.serverType == 0){
                            return "<a href='javascript:;'  onclick='updateServerUser("+ item.data.serverType +")'>修改</a>  <a href='javascript:;'  onclick='changePwd()'>修改密码</a>";// <a href='javascript:;' onclick='getClients()'>分配权限</a> <a href='javascript:;' onclick='deleteServerUser()'>删除</a>";
                        }else{
                            return "<a href='javascript:;'  onclick='updateServerUser("+ item.data.serverType +")'>修改</a>";// <a href='javascript:;' onclick='getClients()'>分配权限</a> <a href='javascript:;' onclick='deleteServerUser()'>删除</a>";
                        }*/
                    }
                }
            ]);
...
//        bodyStyle:'width:100%',
                ...
                tbar:[{
                    ...
                    text:'删除账号',
                    iconCls:'remove',
                    handler:function(){deleteServerUser(grid)}
                }],
                bbar:page_toolbar
            });
            var win = new Ext.Window({
...
//                                                    Ext.getCmp('insert.serverUser').setValue('');
                                            ...
                                            //todo 规则:/服务账号
//                                        Ext.getCmp('insert.serverDir').setValue('/' + serverUser);
                                        }
                                    }
                                });
                            }
                ...
                {fieldLabel:'权限路径',name:'serverUser.dir', allowBlank:false ,emptyText:'请输入权限路径', value:'/data/ftpsource'},
                {fieldLabel:'服务权限',boxLabel: '读(下载)',xtype:'checkbox', name: 'per_read'},
                {height:20,boxLabel: '写(上传)',xtype:'checkbox', name: 'per_write'},
                {height:20,fieldLabel:'',boxLabel: '可执行',xtype:'checkbox', name: 'per_do'}
//                {height:20,fieldLabel:'',boxLabel: '重命名和删除',xtype:'checkbox', name: 'per_delete'}
            ]
        });
    }else{
        formPanel = new Ext.form.FormPanel({
...
//                                        Ext.getCmp('insert.serverDir').setValue('/' + serverUser);
                ...
                {fieldLabel:'权限路径',name:'serverUser.dir', allowBlank:false ,emptyText:'请输入权限路径', value:'/data/smbsource'},
                {fieldLabel:'服务权限',boxLabel: '读(下载)',xtype:'checkbox', name: 'per_read'},
                {height:20,boxLabel: '写(上传)',xtype:'checkbox', name: 'per_write'},
                {height:20,fieldLabel:'',boxLabel: '可执行',xtype:'checkbox', name: 'per_do'}]
        });
...
function updateServerUser(serverType){
    ...
    if(selModel.hasSelection()){
        ...
        Ext.each(selections,function(item){
            var permission=null;
            if(serverType == 0){
                permission = new Ext.form.CheckboxGroup({
                    id:'update.serverUser_permission',
                    fieldLabel:"服务权限",
                    hiddenName:'serverUser.permission',
//                    xtype:'checkboxgroup',
                    autoWidth:true,
                    items: [
                        {boxLabel: '读', id:'update.per_read',name: 'per_read'},
                        {boxLabel: '写', id:'update.per_write',name: 'per_write'},
//                            {boxLabel: '追加',id:'update.per_modify',name: 'per_modify'},
//                        {boxLabel: '新建文件夹',id:'update.per_rename',name: 'per_rename'},
                        {boxLabel: '可执行',id:'update.per_do',name: 'per_do'}
                    ]
                });
            }else{
                permission = new Ext.form.CheckboxGroup({
...
//                    xtype:'checkboxgroup',
            ...
            var formPanel = new Ext.form.FormPanel({
                ...
                items:[
                    ...
                    {fieldLabel:'权限路径',name:'serverUser.dir', allowBlank:false ,emptyText:'请输入权限路径', value:item.data.dir},
                    permission
                    /*{
                        id:'update.serverUser_permission',
                        fieldLabel:"服务权限",
                        ...
                        autoWidth:true,
                        items: [
                            {boxLabel: '读(上传)', id:'update.per_read',name: 'per_read'},
                            {boxLabel: '写(下载)', id:'update.per_write',name: 'per_write'},
//                            {boxLabel: '追加',id:'update.per_modify',name: 'per_modify'},
                            {boxLabel: '新建文件夹',id:'update.per_rename',name: 'per_rename'},
                            {boxLabel: '重命名和删除',id:'update.per_delete',name: 'per_delete'}
                        ]
                    }*/]
            });
            ...
            win.show();
            if(item.data.permission.indexOf('1') > -1){
                Ext.getCmp('update.per_read').setValue(true);
            }
            if(item.data.permission.indexOf('2') > -1){
                Ext.getCmp('update.per_write').setValue(true);
            }
            if(item.data.permission.indexOf('3') > -1){
                Ext.getCmp('update.per_do').setValue(true);
            }
            /*if(item.data.permission.indexOf('4') > -1){
                Ext.getCmp('update.per_rename').setValue(true);
            }
            if(item.data.permission.indexOf('5') > -1){
                Ext.getCmp('update.per_delete').setValue(true);
            }*/
        });
    }
}
                                    ...
                                    success : function(r,o) {
                                        ...
                                        if(msg != 'true'){
                                            Ext.MessageBox.show({
                                                ...
                                                fn:function(e){
                                                    if(e=='ok'){
                                                        Ext.getCmp('old.password.info').setValue('');
                                                    }
                                                }
                                            });
                                        }
                        ...
                        handler:function(){
                            if (formPanel.form.isValid()) {
                                formPanel.getForm().submit({
                                    url :'ServerUserAction_updateUser.action',
                                    method :'POST',
                                    params:{id:item.data.serverId,option:'changePwd'},
                                    waitTitle :'系统提示',
                                    waitMsg :'正在保存,请稍后...',
                                    success : function(form,action) {
                                        var msg = action.result.msg;
...
function deleteServerUser(grid){
    var store = grid.getStore();
    var selModel = grid.getSelectionModel();
    if(selModel.hasSelection()){
        var selections = selModel.getSelections();
