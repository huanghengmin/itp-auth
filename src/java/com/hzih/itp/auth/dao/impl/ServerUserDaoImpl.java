...
public class ServerUserDaoImpl extends MyDaoSupport implements ServerUserDao {
    ...
    public PageResult findByPage(String serverName, String serverType, String serverUser, String dir, String permission, int pageIndex, int limit) {
        ...
        if(ps.getAllResultsAmount() != 0){
            if(serverName == null || serverName.length() <= 0){
                hql += " group by serverName";
            }
            countHql = "select count(*) " + hql;
            ps = this.findByPage(hql,countHql,params.toArray(),pageIndex,limit);
        }
        return ps;
    }
    ...
    @Override
    public PageResult findByServerType(String serverType) {
        String hql = " from ServerUser where 1=1";
        List params = new ArrayList();
        if(serverType != null && serverType.length() > 0){
            hql += " and serverType = ?";
        ...
        PageResult ps = this.findByPage(hql,countHql,params.toArray(),0,500);
        return ps;
    }
    ...
    public List<ServerUser> findByPorperty(String propertyName, String propertyValue,int option) {
        String hql = new String(" from ServerUser where 1=1");
    ...
    public List<ServerUser> findAllServer() {
        String hql = new String(" from ServerUser where 1=1 group by serverName");
        List<ServerUser> serverUsers = getHibernateTemplate().find(hql);
        return serverUsers;
    ...
    public ServerUser findById(int id) {
        String hql = new String(" from ServerUser where id = " + id);
        List<ServerUser> serverUsers = getHibernateTemplate().find(hql);
        if(serverUsers.size() >0 ){
            return serverUsers.get(0);
        }
        return null;
    }
}
